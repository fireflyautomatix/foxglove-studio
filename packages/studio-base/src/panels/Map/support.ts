...
export function parseGeoJSON(
  json: string,
): Array<{ object: GeoJsonObject; style: Record<string, unknown> }> {
  try {
    ...
    return geoJsons.flatMap((geoJson) => {
      switch (geoJson.type) {
        case "Feature": {
          const style: Record<string, unknown> = (geoJson as Feature).properties?.style ?? {};
          return { object: geoJson, style };
        }
        case "FeatureCollection":
          return ((geoJson as Partial<FeatureCollection>).features ?? []).map((feature) => {
            const style: Record<string, unknown> = feature.properties?.style ?? {};
            return { object: geoJson, style };
          });
        default:
          return { object: geoJson, style: {} };
